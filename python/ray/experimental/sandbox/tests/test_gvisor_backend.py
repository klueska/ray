...
class _RecordingImageManager:
    ...
    def _get_overlayfs_image(self, image):
        self.overlayfs_image_calls.append(image)
        return self.image
...
def test_create_sandbox_prepares_overlayfs_sandbox(tmp_path, monkeypatch, mode):
    """create_sandbox mounts an overlayfs sandbox's kernel overlay over its
    cached EROFS image, at root.path resolved against the bundle."""
    captured, manager = _create_until_run(tmp_path, monkeypatch, "overlayfs", mode)

    overlay = captured["rootfs_overlay"]
    assert manager.overlayfs_image_calls == ["busybox:latest"]
...
def test_create_sandbox_erofs_sandbox_skips_overlay(tmp_path, monkeypatch):
    ...
    assert manager.overlayfs_image_calls == []
...
def test_create_sandbox_overlayfs_needs_manager_support(tmp_path, monkeypatch):
    ...
    class _NoOverlayfsManager(_RecordingImageManager):
        ...
        _get_overlayfs_image = BaseImageManager._get_overlayfs_image

    monkeypatch.setattr("shutil.which", lambda name: f"/usr/bin/{name}")
    monkeypatch.setattr(overlayfs, "mount_mode", lambda: overlayfs.MountMode.USERNS)
    manager = _NoOverlayfsManager(image=str(tmp_path / "rootfs.erofs"))
...
def test_overlayfs_sandbox_leaves_cached_image_untouched(overlay_mount_mode):
    """Overlayfs sandboxes boot from an overlay over the cached EROFS image.
    Their writes, and the mount points runsc creates (here an explicit
    workdir on a read-only root), never reach the image or another sandbox.
    The overlay's root matches the image's."""
    backend = GVisorSandboxBackend()
