...
overlayfs mounted on the spec's root.path, in a private mount namespace that
``runsc run`` then runs in. Its read-only lower layer is the image's cached
EROFS image, mounted in that same namespace. Every host-side write to the
rootfs lands in the sandbox's upper layer instead, including OCI hooks' writes
and the mount points runsc creates for the sandbox's mounts.

With mount privilege, the kernel mounts the EROFS image on a loop device, and
the files keep the image's owners. Without it, the overlay is mounted inside a
user namespace of the sandbox's own, and ``erofsfuse`` (from erofs-utils)
serves the image through FUSE. There, the files root owns in the image belong
to the worker, and files any other user owns show up as the overflow user
(nobody), which the host can't modify. That takes Linux 5.11+, a host that
allows unprivileged user namespaces, and access to /dev/fuse.
...
``lower/`` (where the EROFS image is mounted), ``upper/`` and ``work/``.
"""
...
from ray.experimental.sandbox._internal import fs_utils, image_utils
...
class RootfsOverlay:
    ...
    def _mount_script(self) -> str:
        ...
        attributes, so the snippet copies the image root's mode and
        timestamps onto the upper layer, and its owner outside a user
        namespace. Inside one, it leaves ``erofsfuse`` serving the image in
        the background, with its pid in ``$fuse``.
        """
        ...
        # With mount privilege the kernel mounts the image. Inside a user
        # namespace erofsfuse serves it, in the foreground so it stays in the
        # sandbox's process group, and the snippet waits for its mount.
        if self.in_userns:
            mount_image = [
                '{ erofsfuse -f -o allow_other "$image" "$tmpfs/lower" >/dev/null'
                " & fuse=$!; }",
                'for _ in $(seq 100); do mountpoint -q "$tmpfs/lower" && break; '
                'kill -0 "$fuse" 2>/dev/null || break; sleep 0.05; done',
                'mountpoint -q "$tmpfs/lower"',
            ]
        else:
            mount_image = ['mount -t erofs -o ro,loop "$image" "$tmpfs/lower"']

        # Give the overlay's root the image root's attributes. Inside a user
        # namespace the upper layer belongs to the worker, which the image's
        # root maps to.
        copy_root_attrs = [
        ...
        if not self.in_userns:
            copy_root_attrs.append('chown --reference="$tmpfs/lower" "$tmpfs/upper"')

        steps = [
            'image="$1" tmpfs="$2" mnt="$3"',
    ...
    def _mount_script_args(self) -> List[str]:
        """The mount snippet's positional args: the EROFS image, the
        directory to mount the tmpfs on, and the directory to mount the
        overlay on."""
        return [self.image, self.tmpfs_dir, self.mountpoint]
    ...
    def wrap(self, argv: List[str]) -> List[str]:
        ...
        # The script mounts the overlay, drops its own args, and runs argv in
        # the namespaces it mounted into. Inside a user namespace it then
        # stops erofsfuse, so nothing outlives argv. Otherwise it execs argv.
        if self.in_userns:
        ...
        return [
            ...
            *userns,
            ...
            "bash",
            "-c",
            f"{self._mount_script()} && shift {len(script_args)} && {run}",
            "_",
            *script_args,
            *argv,
        ]
...
def has_mount_privilege() -> bool:
    ...
    ``userxattr`` or an EROFS image on a loop device, so this also checks
    for the initial user namespace. Probed once per process.
    """
    try:
...
def can_mount_in_userns() -> bool:
    """Whether this process can mount a kernel overlay over an EROFS image
    inside a user namespace of its own. Probed once per process, by mounting
    a throwaway one."""
    scratch = tempfile.mkdtemp(prefix="ray-overlay-probe-")
    try:
        empty, tmpfs, mountpoint = (
            os.path.join(scratch, d) for d in ("empty", "tmpfs", "mnt")
        )
        for d in (empty, tmpfs, mountpoint):
            os.mkdir(d)
        image = os.path.join(scratch, "probe.erofs")
        image_utils.build_erofs_image(empty, {}, image)

        # Mount an overlay over an empty image exactly the way a sandbox
        # would, and run nothing in it.
...
def mount_mode() -> MountMode:
    ...
    # Without it, erofsfuse serves the image, and the snippet waits for its
    # mount with util-linux's mountpoint.
    missing = [b for b in ("erofsfuse", "mountpoint") if not shutil.which(b)]
    if missing:
        raise SandboxCreationError(
            "Without mount privilege, an overlayfs sandbox serves its image "
            f"with erofsfuse, but PATH has no {', '.join(repr(b) for b in missing)}. "
            "Install erofs-utils built with FUSE support, and util-linux, on "
            "the node image."
        )
    ...
    raise SandboxCreationError(
        ...
        "overlay inside an unprivileged user namespace, over its image served "
        "by erofsfuse, and a test mount of one failed on this node. That "
        "takes Linux 5.11 or later, read-write access to /dev/fuse, and "
...
def prepare(
    *,
    ...
    describe how to mount its kernel overlay.
